...
leftservo.write(95);     
    rightservo.write(88);
    delay(100);
  while((((analogRead(LB)+analogRead(RB))/2) >=930)!=true){
  ...
  void DFS(){
   ...
   // Serial.println(String(wallinfo));
    ...
    while (sp1>0) {
      ...
      if((movePossible(temp_y,temp_x,curr_y, curr_x)!=true)){// if next one next location is not attainable. 
        path[--sp3];
        path[--sp3]; 
        
      Serial.println("TEST:"+String((movePossible(temp_y,temp_x,path[sp3-2], path[sp3-1])!=true)));
...
boolean frontier_contains( int y, int x){
 ...
 void writeWallInfo(){
  ...
  if (wallFront> 300){ front=1;}  /// THESE IF STATEMENTS NEED TO BE REWRITTEN TAKING INTO ACCOUNT PAST VALUES COMPARED TO CURRENT

  if (wallRight> 250){ right=1;}

  if (wallLeft> 250)  {left=1;}
  ...
  else if(curr_direction==3){ walls= (front*2)+(left*2)+(right*8); }
