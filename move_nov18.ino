...
void move(){
  ...
  if (front_middle >= threshold /*&& (analogRead(LF) >= white||analogRead(RF) >= white)*/){ //if at least two sensors are black, move forward
    leftservo.write(103);     
    rightservo.write(85); 
  ...
  else if(front_left<=threshold) /*&& analogRead(M)<=threshold) && analogRead(RF)>=threshold*/{
   leftservo.write(98);     
   rightservo.write(94);
  ...
  else if(front_right<=threshold /*&& analogRead(M)<=threshold) && analogRead(RF)>=threshold*/){
    leftservo.write(94);     
    rightservo.write(89);
