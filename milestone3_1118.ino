    ...
    while (sp1>0) {
     /* Serial.println("while loop");
      ...
      if( temp_y!=path[sp3-2] || temp_x!=path[sp3-1]){
        ...
        if(temp_y!=curr_y || temp_x!=curr_x){move_to(temp_y, temp_x);}
      //curr_y=temp_y;
...
boolean frontier_contains( int y, int x){
 ...
 /*void writeWallInfo(){
  ...
  if (FRONT_INPUT> threshold_front){ front=1;}  /// THESE IF STATEMENTS NEED TO BE REWRITTEN TAKING INTO ACCOUNT PAST VALUES COMPARED TO CURRENT

  if (RIGHT_INPUT> threshold_side){ right=1;}

  if (LEFT_INPUT> threshold_side){  left=1;}
  ...
  wall_info_real[curr_y][curr_x]=walls;
